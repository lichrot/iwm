...
export class IterableWeakMap<K extends WeakKey, V> {
  ...
  readonly #keys: IterableWeakSet<K> = new IterableWeakSet();
  /** Internal WeakMap that doesn't allow iteration. */
  readonly #values: WeakMap<K, V> = new WeakMap();
   ...
   */
  forEach(
    cb: (value: V, key: K, map: IterableWeakMap<K, V>) => void,
    thisArg: unknown,
  ): void {
    for (const entry of this) cb.call(thisArg, entry[1], entry[0], this);
  }
